...
func (V Video) tempRender() Video {
	tempFolder := ""
	file, err := os.CreateTemp(tempFolder, fmt.Sprintf("video-*.%s", V.extension))
...
func Concat(videos []Video) (Video, error) {
	...
	for num, video := range videos {
		...
		if !video.hasModified {
			...
			file, tempFileErr := os.CreateTemp(tempFolder, fmt.Sprintf("video-%d-*.%s", num, video.extension))
...
func Load(fileName string) (Video, error) {
	...
	if errors.Is(err, os.ErrNotExist) {
		return Video{}, fmt.Errorf("error: %s does not exists", fileName)
	}
	if file.IsDir() {
		return Video{}, fmt.Errorf("error: %s is a directory", fileName)
	}
