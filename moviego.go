...
func FromImageAndSound(imageFileName string, soundFileName string, durationSeconds float32) (Video, error) {
	tempFolder := ""
	finalFile, _ := ioutil.TempFile(tempFolder, fmt.Sprintf("final-*.mp4"))
	ffmpeg.Output(
		[]*ffmpeg.Stream{
			ffmpeg.Input(imageFileName),
			ffmpeg.Input(soundFileName),
		},
		finalFile.Name(),
		ffmpeg.KwArgs{"c:v": "libx264", "t": durationSeconds},
	).OverWriteOutput().Run()
	return Load(finalFile.Name())
}
